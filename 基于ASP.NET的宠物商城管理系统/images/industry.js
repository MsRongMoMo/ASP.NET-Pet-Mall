  ...
  function changeLevel(curLevel,classification){  //自适应级数的级联菜单
    ...
    var nextLevel=document.getElementById(nextLevelName);
    if(!nextLevel)
    {
      if(!classification){
        classification="classification";
      }
      setClassification(classification,curLevel.name);
      return; //如果是最后一级，返回；
    }
    ...
    populate_select("/common/ajaxEntry.do?type=siccode&parent_code=" + curLevelTemp,nextLevel,false);
  ...
  function changeCommonLevel(curLevel,classification,type){  //自适应级数的级联菜单
    ...
    newOption.text="请选择...";
    nextLevel.add(newOption);
    populate_select("/common/ajaxEntry.do?type="+type+"&parent_code=" + curLevelTemp,nextLevel,false);
    changeLevel(nextLevel);
}
function changeLevel1(level1S,level2S,level3S,level4S){
  ...
  var level2 = document.getElementById( (level2S==null)?"level2":level2S );
      ...
      populate_select("/common/ajaxEntry.do?type=siccode&parent_code=" + level1Temp,level2,false);
  ...
  var level3 = document.getElementById( (level3S==null)?"level3":level3S );
      ...
      populate_select("/common/ajaxEntry.do?type=siccode&parent_code=" + level2Temp,level3,false);
  ...
  var level4 = document.getElementById( (level4S==null)?"level4":level4S );
      ...
      populate_select("/common/ajaxEntry.do?type=siccode&parent_code=" + level3Temp,level4,false);
  }
  ...
  function setValue(value){
   ...
   pTR = getTR(srcTR.parentElement,srcTR.parentid,srcTR.level-1);
   if (pTR.id == '0'){
       top.frames['result'].setIndustry(srcTR.id);
   }
   else{
       ppTR = getTR(srcTR.parentElement,pTR.parentid,pTR.level-1);
       if (ppTR.id == '0'){
           parent.frames['result'].setIndustry(pTR.id,srcTR.id);
       }
       else {
           pppTR = getTR(srcTR.parentElement,ppTR.parentid,ppTR.level-1);
           if (pppTR.id == '0'){
               parent.frames['result'].setIndustry(ppTR.id,pTR.id,srcTR.id);
           }
           else {
               ppppTR = getTR(srcTR.parentElement,pppTR.parentid,pppTR.level-1);
               if (ppppTR.id == 0){
